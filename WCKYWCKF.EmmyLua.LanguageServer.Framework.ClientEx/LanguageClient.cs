...
public class LanguageClient
{
    ...
    private static readonly global::EmmyLua.LanguageServer.Framework.Server.LanguageServer _jsonProtocolContext =
        global::EmmyLua.LanguageServer.Framework.Server.LanguageServer.From(Stream.Null, Stream.Null);

    private LanguageClient(Stream input, Stream output)
    {
        Reader = new JsonProtocolReader(input, JsonSerializerOptions);
        Writer = new JsonProtocolWriter(output, JsonSerializerOptions);
    }

    public JsonSerializerOptions JsonSerializerOptions { get; } = _jsonProtocolContext.JsonSerializerOptions;
    ...
    private Dictionary<string, Func<NotificationMessage, CancellationToken, Task>> NotificationHandlers { get; } =
        new();

    public ServerCapabilities ServerCapabilities { get; set; } = null!;

    public static LanguageClient From(Stream input, Stream output)
    {
        return new LanguageClient(input, output);
    }
    ...
    // public LanguageClient AddHandler(IJsonHandler handler)
    // {
